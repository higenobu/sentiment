...
def sel_news():
  ...
  try:
    ...
    sql ='select id from dailynews where newsdate='+"'"+tdate+"'"
    #print (sql)
    cur.execute(sql)
...
def ins_news(row,filenm):
  ...
  #cur.execute("set client_encoding to '%s'" % encode)
  try:
    sql = """INSERT INTO dailynews(newsdate,seq,filename,cont,proctime) 
    VALUES(%s,%s,%s,%s,%s) """
    cur.execute(sql, (ndate,aa,filenm,dd,strnow))
    conn.commit()
  except (Exception, psycopg2.DatabaseError) as error:
    print (error)
    print ("ERROR INS-news")
    pass
  finally:
    if conn is not None:
      conn.close()

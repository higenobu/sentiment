  ...
  for nn in nw:
    ...
    if (down<up):
      ...
      if (up<nu):
          ...
          maxval=up
          maxtag='1'+':Down'
    #print ("Max%=",str(maxval/jj)[0:4],"Maxtag=",maxval)
    #cont='1(Down):,'+str(down/jj)[0:4]+',2(Up):,'+str(up/jj)[0:4]+',0(Nu):,'+str(nu/jj)[0:4]+',Total count:,'+str(jj)+',News Date:,'+ddate+',File:,'+ffname+'\n'
    #print (cont)
    #f.write(cont)
    #cont='Maxtag:,'+maxtag+',Max count:,'+str(maxval)+',Max %:,'+str(maxval/jj)[0:4]+','+'News Date:,'+ddate+'\n'
    #print (cont)

    msg="Please see the following Dl result for the next 3days advanced prediction of Nikkei-VI."+ffname+'\n'
    f.write(msg)
    print (msg)
    cont='News Date: '+ddate+'  Maxtag: '+maxtag+'  Max %:  '+str(maxval/jj)[0:4]+'%\n'
    f.write(cont)
    print (cont)
    cont='1(Down):  '+str(down/jj)[0:4]+'%  2(Up):  '+str(up/jj)[0:4]+'%  0(Nu):  '+str(nu/jj)[0:4]+'% \n'
    f.write(cont)
    print (cont)
